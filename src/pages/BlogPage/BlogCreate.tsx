...
const BlogCreate = () => {
  ...
  useEffect(() => {
    const handelGetIdService = async () => {
      const { data } = await axios.get('https://localhost:7212/api/Specialty/' + id)
      console.log(data, 'pl')
      setDataService(data)
    }
    handelGetIdService()
  ...
  const onFinish = (values: any) => {
    ...
    if (!id) {
      axios
        .post('https://localhost:7212/api/Specialty', dataBody)
        .then((items: any) => {
      ...
      axios
        .put('https://localhost:7212/api/Specialty/' + id, dataBody)
        .then((items: any) => {
          window.location.href = '/manager/blogs'
        })
        .catch((e) => console.log(e))
